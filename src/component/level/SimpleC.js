...
const SimpleC = () => {
  ...
  const complition = () => {
    if (Object.keys(matchedCards).length === array.length) {
      setStartTimer(false)
      setPlaylist([...playlist, { level: 'Simple', time: time }])
      setShowModal(true);
    }
  };

  /* compare card when two card opened 
    - when compare then put card value in matched card
    - not matched then put empty array in opencard*/
  const compare = () => {
    const [a, b] = openCards;
    enable();
    if (cards[a] === cards[b]) {
      ...
      <Container component={Box} className={classes.container}>
        <Grid container spacing={2}>
          <Grid item md={12} sm={12}>
            <Typography variant='h2' className={classes.pageTitle}>😄 Mix-or-Match 😄</Typography>
          </Grid>
          <Grid item md={6} sm={6}>
